...
export const getChainLevel = (mirror: MirrorTexture): number => {
  ...
  while (current.renderList && current.renderList.length > 0 && level < MAX_MIRROR_LEVELS) {
    // In tests, we can't use instanceof, so check for renderList property
    const nextMirror = current.renderList.find(item => 
      item && typeof item === 'object' && 'renderList' in item
