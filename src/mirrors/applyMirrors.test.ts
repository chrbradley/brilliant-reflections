...
import { applyMirrorToWall, getMirrorWalls, createMirrorMaterial } from './applyMirrors';
...
describe('applyMirrors', () => {
  ...
  describe('applyMirrorToWall', () => {
    ...
    it('should not render the wall itself in mirror', () => {
      ...
      // In test environment, mirror texture creation may fail
      if (result.material.reflectionTexture && result.material.reflectionTexture instanceof MirrorTexture) {
        const mirrorTexture = result.material.reflectionTexture as MirrorTexture;
    ...
    it('should include other meshes in render list', () => {
      ...
      // Create a test mesh
      const testMesh = BABYLON.MeshBuilder.CreateBox('testBox', { size: 1 }, sceneConfig.scene);
